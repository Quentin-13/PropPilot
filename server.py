...
def _trigger_post_extraction_hooks(lead_id: str, client_id: str) -> None:
    """
    Hooks post-extraction SMS : calcul next_action PUIS push CRM.
    ...
    try:
        ...
        if lead_data:
            if not next_action_fresh:
                ...
                logger.warning("[PostExtraction] next_action non recalculée — champs action neutralisés lead_id=%s", lead_id)
            push_lead_to_crm(client_id=client_id, lead_data=lead_data)
        else:
            logger.warning("[PostExtraction] lead_data introuvable — push CRM skippé lead_id=%s", lead_id)
    except Exception as e:
    ...
    for item in active:
        ...
        try:
            ...
            if data:
                ...
                _trigger_post_extraction_hooks(lead_id=lead_id, client_id=client_id)
        except Exception as e:
